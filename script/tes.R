...
numerik <- numerik %>% select(-tahun)

statistik_deskriptif <- data.frame(
  Variabel = names(select(numerik, where(is.numeric))),
  Mean = sapply(select(numerik, where(is.numeric)), mean, na.rm = TRUE),
  Median = sapply(select(numerik, where(is.numeric)), median, na.rm = TRUE),
  Min = sapply(select(numerik, where(is.numeric)), min, na.rm = TRUE),
  Q1 = sapply(select(numerik, where(is.numeric)), quantile, probs = 0.25, na.rm = TRUE),
  Q2 = sapply(select(numerik, where(is.numeric)), quantile, probs = 0.50, na.rm = TRUE),
  Q3 = sapply(select(numerik, where(is.numeric)), quantile, probs = 0.75, na.rm = TRUE),
  Max = sapply(select(numerik, where(is.numeric)), max, na.rm = TRUE),
  SD = sapply(select(numerik, where(is.numeric)), sd, na.rm = TRUE),
  Varians = sapply(select(numerik, where(is.numeric)), var, na.rm = TRUE)
)
